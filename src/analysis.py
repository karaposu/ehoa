...
def find_category_and_subcategory(desc, llm, llm_refiner, categories_and_rules, list_of_main_categories,
    ...
    if classification_result is None:
        ...
        if keyword_based_classification_result is None:
            ...
            classification_result, raw_string_answer, refined_answer = classify_and_refine(llm, llm_refiner, desc, categories_and_rules,
                                                        list_of_main_categories, list_of_subcategories)
            if classification_result is None:
                return {'category': 'Uncategorized', 'subcategory': 'Uncategorized'}, by_bank_pattern, by_keyword_rule , raw_string_answer,refined_answer
...
def do_analysis(llm, llm_refiner, pdf_document_path, bank_name):
    ...
    table_df["desc"] = descs
    categories_and_rules, list_of_main_categories, list_of_subcategories = get_hierarchical_categories('categories.yaml')
    ...
    for e in tqdm(descs, desc="Classifying records"):
        ...
        if string_id in classification_cache:
            ...
            classification_result, by_bank_pattern, by_keyword_rule,  raw_string_answer, refined_answer = find_category_and_subcategory( e,
                                                                                                                                         llm,
                                                                                                                                         llm_refiner,
